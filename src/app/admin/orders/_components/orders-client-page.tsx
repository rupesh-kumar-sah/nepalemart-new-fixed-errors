...
import { MoreHorizontal, ArrowUpDown, CheckCircle, Clock, XCircle, AlertTriangle, Trash2, ShoppingBag, Truck, PackageCheck } from 'lucide-react';
...
export function OrdersPageClient({ initialOrders }: { initialOrders: Order[] }) {
  ...
  const shipOrder = (orderId: string) => updateOrderStatus(orderId, 'shipped', 'Order Shipped', `Order #${orderId} has been marked as shipped.`);
  const deliverOrder = (orderId: string) => updateOrderStatus(orderId, 'delivered', 'Order Delivered', `Order #${orderId} has been marked as delivered.`);
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      {order.status === 'pending' && (
                       ...
                       {order.status === 'confirmed' && (
                        <DropdownMenuItem onClick={() => shipOrder(order.id)}>Mark as Shipped</DropdownMenuItem>
                      )}
                      {order.status === 'shipped' && (
                        <DropdownMenuItem onClick={() => deliverOrder(order.id)}>Mark as Delivered</DropdownMenuItem>
                      )}
                      <DropdownMenuSeparator />
