...
export function ProductsPageClient({ initialProducts }: { initialProducts: Product[] }) {
  ...
  const { isOwner, isMounted } = useAuth();
  ...
  useEffect(() => {
    // This allows the server-rendered initialProducts to be updated
    // on the client if a revalidation occurs (e.g., after an add/edit).
