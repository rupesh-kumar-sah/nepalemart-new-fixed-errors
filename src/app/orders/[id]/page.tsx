...
import { MessageSquare, CheckCircle, Clock, Trash2, ArrowLeft, AlertTriangle, XCircle, Loader2, Truck, PackageCheck } from 'lucide-react';
...
export default function OrderDetailPage() {
  ...
  const shipOrder = (orderId: string) => {
    updateOrderStatus(orderId, 'shipped', 'Order Shipped', `Order #${orderId} has been marked as shipped.`);
  };

  const deliverOrder = (orderId: string) => {
    updateOrderStatus(orderId, 'delivered', 'Order Delivered', `Order #${orderId} has been marked as delivered.`);
  };
  ...
  const getPaymentStatus = () => {
    switch (order.status) {
      ...
      case 'shipped':
      case 'delivered':
        return { text: 'Payment Confirmed', className: 'text-green-600' };
      case 'payment-issue':
        return { text: 'Payment Rejected', className: 'text-destructive' };
      case 'cancelled':
        return { text: 'Order Cancelled', className: 'text-muted-foreground' };
      case 'pending':
      default:
        return { text: 'Pending Verification', className: 'text-amber-600' };
    }
  };
            ...
            <CardHeader className="flex-col md:flex-row justify-between items-start md:items-center">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className={cn(
                        ...
                        {order.status === 'delivered' && <PackageCheck className="h-4 w-4" />}
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                            ...
                            <p><strong>Status:</strong> <span className={cn("font-medium", paymentStatus.className)}>{paymentStatus.text}</span></p>
                        </div>
                    </div>
                </div>
                <Separator />
